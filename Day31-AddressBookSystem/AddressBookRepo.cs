    ...
    public class AddressBookRepo
    {
            ...
            try
            {
                ...
                using (this.connection)
                {
                    using (SqlCommand command = new SqlCommand(
                        @"SELECT * FROM address_books WHERE city = 'Sathya' OR state = 'Andhrapradesh'; 
                            SELECT * FROM address_books WHERE city = 'Adoni' OR state = 'Andhrapradesh';", connection))
                    {
                        connection.Open();
        ...
        public void AddressBookSizeByCityANDState()
        {
            try
            {
                ...
                {
                    using (SqlCommand command = new SqlCommand(
                        @"select count(first_name) from address_books WHERE city = 'Adoni' AND state = 'Andhrapradesh'; 
                        select count(first_name) from address_books WHERE city = 'Nandyala' AND state = 'Andhrapradesh';", connection))
                    {
                        connection.Open();
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                Console.WriteLine("Total Contacts From City Adoni And State Andhrapradesh: {0}", +count);
                            }
                            if (reader.NextResult())
                            {
                                while (reader.Read())
                                {
                                    int count = reader.GetInt32(0);
                                    Console.WriteLine("Total Contacts From City Nandyala And State Andhrapradesh: {0}", +count);
                                }
                            }
                        }
                        connection.Close();
        ...
        public void SortPersonNameByCity()
        {
            try
            {
                ...
                {
                    using (SqlCommand command = new SqlCommand(
                        @"SELECT * FROM address_books WHERE city = 'Adoni' order by first_name; 
                        SELECT * FROM address_books WHERE city = 'Nandyala' order by first_name, last_name;", connection))
                    {
                        connection.Open();
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
